...
for i_s = which_subjects
    ...
    if blockNo ==0
        ...
        [mean_beta_vec,detection_vec,stimulus_vec,response_vec,...
            RT_vec,confidence_vec, id, npress_vec] = deal([]);
        type_vec = {};
        ...
        for i_b = 1:numel(names)
            ...
            if regexp(regressor_name,'response_(?<response>\d+)')
                ...
                matchexp = regexp(regressor_name, ...
                    strcat('Sn\((?<run_num>\d+)\) (?<trial_type>\w+)_detection_(?<detection>\d+)',...
                    '_stimulus_(?<stimulus>\d+)_response_(?<response>\d+)',...
                    '_confidence_(?<confidence>\d+)_RT_(?<RT>\d+.\d+)_id(?<id>\d+)'),...
                    'names');
                
                
                detection_vec(end+1) = str2num(matchexp.detection);
                ...
                if str2num(matchexp.run_num)~=run_number
                    ...
                    events_file = fullfile(project_params.data_dir,'..','data',...
                        ...
                        ['sub-',subj{i_s}.scanid,'_task-detectdiscrim_run-0',...
                ...
                sanity_check = strcmp(matchexp.detection,table.detection(i_t)) &...
        ...
        save(fullfile(p.stats_dir,...
            ...
            'mean_beta_vec', 'detection_vec','stimulus_vec','response_vec',...
            'RT_vec','confidence_vec', 'id', 'type_vec', 'npress_vec','coordinates');
